...
import yaml, os, sys, numpy as np
import importlib
...
def evaluate_regression_model(model_list: list, X_train:np.ndarray, y_train:np.ndarray, X_test:np.ndarray, y_test:np.ndarray, base_accuracy:float=0.6) -> MetricInfoArtifact:
    ...
    try:
        ...
        for model in model_list:
            ...
            print(f"--\n--------evaluate_regression_model----->>>>>X_train:  >>>-{len(X_train)}----->>X_test::>>>>{len(X_test)}<<<<<<<----evaluate_regression_mode---\n--")
            
            print(f"--\n--------evaluate_regression_model----->>>>>train:  >>>-{len(y_train_pred)}----->>test::>>>>{len(y_test_pred)}<<<<<<<----evaluate_regression_mode---\n--")

            #Calculating r squared score on training and testing dataset
            train_acc = r2_score(y_train, y_train_pred)
            test_acc = r2_score(y_test, y_test_pred)
            print(f"----------evaluate_regression_model----->>test-: {test_acc}-----train : >>{train_acc}<<<<<<<----evaluate_regression_mode-----")
            #Calculating mean squared error on training and testing dataset
            train_rmse = np.sqrt(mean_squared_error(y_train, y_train_pred))
            test_rmse = np.sqrt(mean_squared_error(y_test, y_test_pred))
            print(f"----------evaluate_regression_model----->>train_rmse-: {train_rmse}----test_rmse : >>{test_rmse}<<<<<<<----evaluate_regression_mode-----")
            
            # Calculating harmonic mean of train_accuracy and test_accuracy
            model_accuracy = (2 * (train_acc * test_acc)) / (train_acc + test_acc)
            print(f"------evaluate_regression_model----->>>>>>>> {model_accuracy} <<<<<<<<---------model_accuracy---------")
            diff_test_train_acc = abs(test_acc - train_acc)
            print(f"------evaluate_regression_model----->>>>>>>> {diff_test_train_acc} <<<<<<<<---------diff_test_train_acc--------")
            ...
            if model_accuracy >= base_accuracy and diff_test_train_acc < 0.05:
                ...
                print(f"------evaluate_regression_model----->>>>>>>> {model_accuracy} <<<<<<<<---------model_accuracy >= base_accuracy--------")
                ...
                print(f"------evaluate_regression_model----->>>>>>>> {metric_info_artifact} <<<<<<<<--------metric_info_artifact--------")

            elif model_accuracy > 0:
                metric_info_artifact = MetricInfoArtifact(model_name=model_name,
                                                        model_object=model,
                                                        train_rmse=train_rmse,
                                                        test_rmse=test_rmse,
                ...
                print(f"----metric_info_artifact--evaluate_regression_model----->>>>>>>> {metric_info_artifact} <<<<<<<<--------model_accuracy > 0--------")
            index_number += 1
...
class ModelFactory:
    def __init__(self, model_config_path: str = None,):
        try:
            ...
            print(f"---ModelFactory--init--------->>> {self.config} <<<<<<<<<---self.config: dict-----------")
            self.grid_search_cv_module: str = self.config[GRID_SEARCH_KEY][MODULE_KEY]
            print(f"---ModelFactory--init--------->>> {self.grid_search_cv_module} <<<<<<<<<--self.grid_search_cv_module-----------")
            ...
            print(f"---ModelFactory--init--------->>> {self.grid_search_property_data} <<<<<<<<<---self.grid_search_property_data-----------")

            self.models_initialization_config: dict = dict(self.config[MODEL_SELECTION_KEY])

            self.initialized_model_list = None
            self.grid_searched_best_model_list = None
            print(f"---ModelFactory--init--------->>> {self.models_initialization_config} <<<<<<<<<---self.models_initialization_config-----------")
    ...
    def update_property_of_class(instance_ref:object, property_data: dict):
        try:
            ...
            print(property_data)
            print(f"---ModelFactory-update_property_of_class------->>> {property_data} <<<<<<<<<---property_data----------")
            
            for key, value in property_data.items():
                logging.info(f"Executing:$ {str(instance_ref)}.{key}={value}")
                setattr(instance_ref, key, value)
            print(f"---ModelFactory-update_property_of_class------->>> {instance_ref} <<<<<<<<<---instance_ref---------")
    ...
    def read_params(config_path: str) -> dict:
        try:
            with open(config_path) as yaml_file:
                ...
                print(f"-----modelfactro --read_params---------->>> {config} <<<<<<<<<-------config--------")
    ...
    def class_for_name(module_name:str, class_name:str):
        try:
            ...
            print(f"---ModelFactory-class_for_name------->>> {class_ref} <<<<<<<<<---class_ref---------")
    ...
    def execute_grid_search_operation(self, initialized_model: InitializedModelDetail, input_feature,
        ...
        try:
            ...
            print(f"---ModelFactory execute_grid_search_operation------>>> {grid_search_cv} <<<<<<<<<---grid_search_cv-------")
            ...
            print(f"---ModelFactory execute_grid_search_operation------>>> {grid_searched_best_model} <<<<<<<<<--grid_searched_best_model-------")
    ...
    def get_initialized_model_list(self) -> List[InitializedModelDetail]:
        ...
        try:
            ...
            for model_serial_number in self.models_initialization_config.keys():
                print(f"----get_initialized_model_list----->>>>> {self.models_initialization_config.keys()} <<<<<<<<_-----self.models_initialization_config.keys()-----")
                ...
                model = model_obj_ref()
                print(f"---ModelFactory get_initialized_model_lis------>>> {model} <<<<<<<<<------model-------")
    ...
    def get_model_detail(model_details: List[InitializedModelDetail],
        ...
        try:
            for model_data in model_details:
                print(f"-get_modeldetail---modelfactro----{model_details}-------model_detail------")
        ...
        try:
            ...
            for grid_searched_best_model in grid_searched_best_model_list:
                if base_accuracy < grid_searched_best_model.best_score:
                    ...
                    """
                if grid_searched_best_model.best_score:
                    logging.info(f"Acceptable model found:{grid_searched_best_model}")
                    print(f"---model factory--grid_searched_best_model_list---->>>{grid_searched_best_model.best_score}<<<<<<---grid_searched_best_model.best_score---")
                    base_accuracy = grid_searched_best_model.best_score
                    best_model = grid_searched_best_model.best_score
    ...
    def get_best_model(self, X, y,base_accuracy=0.6) -> BestModel:
        try:
            ...
            print(f"---get_best_model--modelfactro----{initialized_model_list}------initialized_model_list------")
